...
#CIFS2move=[ 'purchasing', 'system', 'assistants', 'distrib']
#CIFS2move=['wpkg']
CIFS2move=['design']
...
fnas_from = fnas('http://fnas-alpha-adm.arc.world/api/v1.0')
fnas_to = fnas('http://fnas-beta.arc.world/api/v1.0')
...
# FROM
    ...
    for t in fnas_from.SnapTask_List(fs):
        t["task_enabled"] = False
        print('SnapTask_Update')
        fnas_from.SnapTask_Update(t["id"], t)
        print('done')
    task = fnas_from.ReplTask_Get('fnas-beta.arc.world', fs)
    task["repl_enabled"] = False
    print('ReplTask_Update')
    fnas_from.ReplTask_Update('fnas-beta.arc.world', fs, task)
    print("done")
# TO
